...
import { addItem } from '../slice/cartSlice'; // Adjust the import path if necessary
import NavBar from '../components/NavBar';
import Footer from '../components/Footer';
import { fetchRecommendations } from '../slice/recommendationSlice'; // Import fetchProducts thunk

const RecommendationDetails = () => {
  const { id } = useParams();
  const dispatch = useDispatch();

  const { items: recommendations, status, error } = useSelector((state) => state.recommendations);
  const product = recommendations.find((product) => product.id === Number(id));
